...
- 页面按“国家/地区 × 套餐”矩阵展示：国家一行，Go、Plus、Pro 5x、Pro 20x 等套餐各占一列；新套餐会自动追加列，静态首页与桌面表格宽度随套餐数扩展。**主表和排名只使用每个同名套餐在该地区的最低公开标价**；这个比较价只由该套餐自身决定，不引用其他套餐、不使用倍数或价差启发式，也不推断计费周期、促销或购买资格。完整同名金额始终保留在 JSON 与价格历史中。顶部按人民币参考价显示各套餐在已覆盖地区中的当前最低价；并列最低超过 3 个地区时只显示“X 个地区并列最低”。
...
生产自动更新采用主备：**Cloudflare 每日北京时间 09:05** 外部调用 `workflow_dispatch`（`trigger_source=cloudflare`）作为主触发，**GitHub cron 09:35** 提供一次低成本兜底；也支持手动运行。GitHub cron 可能延迟，因此不把 09:35 当作严格 SLA。自动任务在真正开始执行时解析最新 `main`，避免排队中的备用任务继续使用过时事件快照。两个自动入口先校验当前 `prices.json` 和当天 `Update ChatGPT prices` 的成功生产运行证明：只有数据全部 `verified`、汇率仍新鲜，且有一条在当前数据生成之后完成的完整成功运行时，备用触发才跳过；若存在 `retained` / `pending` / `unavailable`、fallback 汇率、旧数据、主触发失败，或数据虽已提交但 Pages/生产验证未完成，GitHub 备用仍继续重试。若 Cloudflare 主触发已经形成成功生产证明，09:35 的 GitHub 备用会由 daily guard 自动跳过。手动运行不受每日幂等跳过。仓库只能验证这一调度契约，不能单独证明 Cloudflare 控制面的实时启用状态。
