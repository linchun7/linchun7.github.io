...
class ContractTests(unittest.TestCase):
    ...
    def test_static_projection_and_escape(self):
        ...
        page_revision = re.search(r'<meta name="chatgpt-page-revision" content="([a-f0-9]{64})">', page)
        self.assertIsNotNone(page_revision)
        changed_template = template.replace('</head>', '<!-- projection-change --></head>')
        changed_page = p.render(d, changed_template)
        changed_revision = re.search(r'<meta name="chatgpt-page-revision" content="([a-f0-9]{64})">', changed_page)
        self.assertIsNotNone(changed_revision)
        self.assertNotEqual(page_revision.group(1), changed_revision.group(1))
        self.assertIn('data-lucide="arrow-up"',page)
        self.assertIn('mobile-rank-sr visually-hidden',page)
        self.assertIn('全球价格排名第 1',page)
        self.assertNotIn('已覆盖地区价格排名第',page)
        self.assertIn('各套餐全球最低价',page)
        self.assertIn('全球 ChatGPT App Store 标价',page)
        self.assertRegex(page, r'<dd id="marketCount">\d+ 个地区</dd>')
