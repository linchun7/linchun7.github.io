...
(function(global, factory) {
	...
	var NAME_SUFFIX = `(?:(?<![A-Za-z0-9])(?:(?:Apple TV|CATCHPLAY|[Dd]iscovery|Disney|ESPN|Fitness|iCloud|Paramount|PS|Pro)\\+|(?:(?:tw)?(?:AA|BBB|BB|CCC)|tw[AB]|AB|RhD|Rh)[+-])|(?:公視|影劇館)\\+)`;
	var NAME_SUFFIX_AT_END = new RegExp(`${NAME_SUFFIX}$`);
	var CLOSING_AFTER_SUFFIX = /[/)\]}\uff09\u3011\u3015\u3009\u300b\u300d\u300f\uff0c\u3002\u3001\uff1b\uff1a\uff01\uff1f]/;
	var CJK_OPERATOR_ANS = new RegExp(`([${CJK}])([${OPERATORS}])([${AN}])`, "g");
	...
	function decideBoundarySpacing(boundarySpacingContext) {
		if (boundarySpacingContext.spaceLikeSiblingAfterCurrent) return "none";
		if (boundarySpacingContext.currentEndsWithSpace || boundarySpacingContext.nextStartsWithSpace || boundarySpacingContext.whitespaceBetween) return "none";
		...
		if (boundarySpacingContext.spaceLikeSiblingAfterCurrentBoundary || boundarySpacingContext.currentBoundaryIsBlock) return "none";
		if (!boundarySpacingContext.nextBoundaryIsSpaceSensitive) {
			if (boundarySpacingContext.nextBoundaryIsIgnored || boundarySpacingContext.nextBoundaryIsBlock || boundarySpacingContext.spaceLikeSiblingBeforeNext || boundarySpacingContext.hiddenBoundaryBefore()) return "none";
			return "prepend-next";
		}
		if (!boundarySpacingContext.currentBoundaryIsSpaceSensitive) {
			if (boundarySpacingContext.hiddenBoundaryAfter()) return "none";
			return "append-current";
		}
		if (boundarySpacingContext.spaceLikeSiblingBeforeNextBoundary || boundarySpacingContext.hiddenBoundaryAfter()) return "none";
	...
	*/
	...
	var BrowserPangu = class BrowserPangu extends Pangu {
		...
		spaceNodeSync(contextNode, maxTextNodes) {
		...
		scanBetweenTextNodes(currentBoundaryNode, nextBoundaryNode) {
