...
生产更新先完成 Apple 抓取、双解析器校验、发布日期和完整性校验、汇率校验及价格异常检查，再保存规范化 JSON。仅页面发布日期变化而规范化价格内容不变时，不新增快照并沿用上一公开发布日期；相同发布日期且内容指纹相同的页面不会重复保存；相同发布日期但规范化内容变化时，生成新的 `YYYY-MM-DD-<hash>.json` 修订，旧修订不覆盖。
