...
- `htmlSha256` / `dataSha256`：原始 HTML 和规范化 JSON 文件字节的 SHA-256，用于快速发现证据缺失或被修改。
...
生产快照不是单一 staging 目录提交：HTML 使用排他创建，JSON 通过同目录临时文件和硬链接排他落盘，`index.json` 使用原子替换；任一快照写入失败只清理本次实际创建的文件，外层更新器还会恢复价格、历史、运行日志和原索引文本。生产任务成功后，活动修订是该发布日期中 `firstConfirmedDate` 最新的修订。

日常校验会读取并核对所有证据的 `htmlSha256` 和 `dataSha256`，但只深度解析最新活动修订；缺少原始哈希的旧索引会自动执行深度校验。`pnpm validate:snapshots` 会深度解析全部修订，并由手动及每周只读工作流执行。
