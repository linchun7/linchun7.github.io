...
test('keeps the scheduled update workflow guarded and ordered', async () => {
  ...
  assert.match(workflow, /cron:\s*['"]57 9 \* \* \*['"]/, 'temporary verification schedule must run at 17:57 Beijing time');
  ...
  assert.match(workflow, /name: 记录浏览器界面测试警告[\s\S]*?steps\.ui_before\.outcome == 'failure'[\s\S]*?steps\.ui_after\.outcome == 'failure'/);
