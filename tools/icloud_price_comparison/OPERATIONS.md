...
这项门禁不是“每次更新都抓两遍”。第一次完整抓取经过双解析器交叉核对后：

- 仅价格、币种、分区或发布日期变化：继续正常校验和发布，不增加第二次 Apple 网络抓取。
- 地区或容量新增/移除：执行独立、无缓存的完整确认抓取；确认抓取使用与主抓取相同的重试序列，并受同一个 5 分钟网络预算限制。
...
- 公开 Markdown 受 `AGENTS.md` 约束；新增文档必须是长期维护的用户/运维/数据格式/贡献/许可说明，不得提交聊天交接、临时状态或内部签字材料。
