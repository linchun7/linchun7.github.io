...
已由两份 Apple `cross-checked` 样本确认、且仍在 0.1x–10x 硬边界内的大幅历史变价以 `PRICE_CHANGE_ANOMALY_CONFIRMED` warning 自动继续；非法/缺失价格、parser disagreement、20x 当前 CNY 市场 outlier 等仍 fail closed。FX authenticated candidate sanity 失败后自动尝试 open endpoint；所有 fresh online candidates 都失败时，仅可沿用现有 36 小时窗口内且覆盖要求的 previous FX/CNY 数据，并记录 `FX_SANITY_FALLBACK_TO_PREVIOUS`。stale success 不满足 daily guard，备用任务仍会自动重试；没有安全 previous fallback 时按 transient 失败，绝不发布 sanity 异常的新 FX。
### 完整只读验证
