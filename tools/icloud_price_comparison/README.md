...
- 使用两套独立 DOM 路径解析 Apple 页面；结果一致时交叉确认，单路径可用时明确标记降级
- 使用更宽松的极端变化阈值，并结合当地价格、固定汇率人民币差额和实际汇率人民币差额判断可疑调价
...
计划时间：每天北京时间 17:57（UTC 09:57）。GitHub 定时任务是尽力而为，仍可能延迟数分钟甚至数小时。

每次运行按以下顺序执行：

1. 安装锁定版本的依赖。
2. 使用固定 Apple HTML fixture 和全球历史调价 fixture 运行解析、价格阈值、历史、工作流和当前数据完整性测试。
3. 请求 Apple 官方页面，使用文档语义顺序和 Apple 区域/标题标记两条独立路径解析。
4. 两套结果均可用时必须完全一致；再校验地区数量、重复地区、容量完整性和异常价格跳变。
5. 获取汇率；汇率服务失败时沿用上次成功结果并标记为过期。
6. 比较当前数据与上一份有效数据，更新价格历史、发布日期历史和结构化运行日志。
7. 使用原子写入更新 JSON，再验证新数据完整性并运行一次真实浏览器 UI 测试。
...
失败任务不会写入成功日志；失败原因、耗时和调用栈写入该次 Actions 的 `run-report.json` 附件。

## 稳定性保护

- Apple 请求最长 45 秒，最多五次退避重试
- 响应体过小或缺少关键价格内容时拒绝解析
- 两套解析结果同时成功但内容不一致时拒绝更新
- 单解析器降级时必须继续通过全部数据、汇率和历史变化校验，并在摘要与日志中明确记录
- 关键分区缺失、地区重复、容量价格不完整时拒绝更新
- 地区数量异常下降时拒绝更新
- 同币种单项价格出现超过十倍的极端倍数变化时拒绝更新
- 当地价格涨幅达到 200%，且当地金额、按上一份已验收汇率计算的人民币差额、按当日汇率计算的实际价值差额均超过阈值时拒绝更新，等待人工复核。人民币阈值取 `15 元`与该容量上次人民币价值的 `50%` 两者较高值，因此会随容量价格自动放大；如果 Apple 调价主要用于抵消汇率剧烈波动，或仅有汇率变化而当地标价未变，实际汇率口径会放行
...
- 绿色：抓取前核心回归、抓取后数据与页面验收、数据提交均成功。摘要显示地区数、价格点数、Apple 日期、汇率时间和变更统计。
- 红色：先查看失败步骤，再下载 `icloud-price-diagnostics-*` 附件。
- `apple-response-<UTC 时间>.html`：本次 Apple 原始页面，用于复现解析问题。
- `run-report.json`：结构化失败状态、耗时和错误。
- 工作流摘要同时显示 Git 历史和 `history.json` 当前体积；Git 历史达到 500 MiB 或价格历史达到 2 MiB 时产生警告。
...
1. **Apple 数据来源**：两套解析器都只读取同一个 Apple 官方页面，不自动切换到非官方价格源。两套结果分歧时宁可保留旧数据，也不选择性猜测。
2. **容量控制**：`run-log.json` 固定保留 90 次，HTML 只保留为 14 天附件，价格历史只在变化时增加。Git 历史达到 500 MiB 时仅预警，不自动改写历史；任何清理都必须人工确认。
