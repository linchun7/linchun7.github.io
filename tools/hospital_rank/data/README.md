...
- `source-snapshot.json`：2026-08-18 从健康界复旦排行榜逐年页面重新抓取并规范化得到的源站结构化快照。**不保存原始 HTML**。
- `audit.json`：本次迁移审计记录，包括旧数据核对、更名归并、缺失年度恢复、源站内部校验 warning 等。
...
- `hospitalId`：对应医院实体。
- `sourceName`：**该年度源站实际显示的医院名称**，用于保留来源事实和追溯更名。
